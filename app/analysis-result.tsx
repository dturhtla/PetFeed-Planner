    ...
    try {
      ...
      try {
        const foodResponse = await fetch(`${GO_SERVER_URL}/api/v1/foods`, {
          ...
          body: JSON.stringify({
            fat_pct: foodInfo.fat_pct,
            kcal_per_g: foodInfo.calories_per_100g
              ? foodInfo.calories_per_100g / 100
              : null,
            product_name: foodName,
            protein_pct: foodInfo.protein_pct,
          }),
        });
        ...
        if (foodId) {
          await fetch(`${GO_SERVER_URL}/api/v1/pets/${savedPetId}/analysis`, {
            method: "PATCH",
            headers: {
              "Content-Type": "application/json",
              "ngrok-skip-browser-warning": "true",
            ...
            body: JSON.stringify({
              food_id: foodId,
              recommended_amount: oneMealAmount,
            }),
          });

          console.log("대표 사료 변경 완료");
        }
      } catch (err) {
        console.log("사료 등록 또는 대표 사료 변경 실패:", err);
        ToastAndroid.show("사료 저장에 실패했어요.", ToastAndroid.SHORT);
        return;
      }
