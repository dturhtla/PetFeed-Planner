...
주의사항:
- 특이한 폰트, 기울어진 글씨, 반사광이 있어도 최대한 읽어주세요
- 브랜드명은 로고 폰트 그대로 읽어주세요
- 포장지 전체를 꼼꼼히 스캔해서 성분표를 찾아주세요
- 성분표가 작게 인쇄되어 있어도 최대한 읽어주세요
- 영어, 한국어 혼용 표기도 정확히 읽어주세요
- 확실하지 않아도 최선의 추정값을 반환해주세요
- null보다는 추정값을 반환하는 것이 좋아요
...
- 칼로리 변환 규칙: kcal/kg 값을 반드시 10으로 나눠서 100g 단위로 변환
- 예: 3525kcal/kg → 352.5kcal/100g, 4000kcal/kg → 400kcal/100g
...
- 나머지 텍스트 값은 한국어로 반환해주세요"""
    ...
    # 칼로리, 단백질, 지방 병렬 검색
    need_calories = food.calories_per_100g is None and food.product_name and food.brand
    need_protein = food.protein_pct is None and food.product_name and food.brand
    need_fat = food.fat_pct is None and food.product_name and food.brand

    if need_calories or need_protein or need_fat:
        tasks = [
            search_product_calories(food.brand, food.product_name) if need_calories else asyncio.sleep(0),
            search_product_nutrition(food.brand, food.product_name, "조단백질") if need_protein else asyncio.sleep(0),
            search_product_nutrition(food.brand, food.product_name, "조지방") if need_fat else asyncio.sleep(0),
        ]

        results = await asyncio.gather(*tasks)

        if need_calories:
            food.calories_per_100g = results[0]
        if need_protein:
            food.protein_pct = results[1]
        if need_fat:
            food.fat_pct = results[2]
...
예시:
브랜드: ROYAL CANIN, 제품명: KITTEN → 407
브랜드: 하림펫푸드, 제품명: 밥이보약 빛나는 피모 → 352.5
브랜드: 퓨리나, 제품명: PRO PLAN ADULT → 385
...
예시:
브랜드: ROYAL CANIN, 제품명: KITTEN, 조단백질 → 33.0
브랜드: ROYAL CANIN, 제품명: KITTEN, 조지방 → 20.0
브랜드: 하림펫푸드, 제품명: 밥이보약 빛나는 피모, 조단백질 → 25.0
브랜드: 하림펫푸드, 제품명: 밥이보약 빛나는 피모, 조지방 → 12.0
